...
| v8 | Jan 4, 2026 | Added beta reader signup system and M3 preview page |
...
│   ├── murder-malady-monsters.html  # M3 first 3 chapters preview (added Jan 4, 2026)
│   ├── css/
│   │   └── style.css
│   ├── images/
│   │   ├── dlyh-concept.png
...
| Class | Color | Usage |
|-------|-------|-------|
| .btn-primary | Cyan | Primary actions (Read Now, Play, etc.) |
| .btn-outline | Cyan outline | Secondary actions |
| .btn-beta | Red | Beta reader signup |
...
<button class="btn btn-beta">Beta Read</button>
```
...
**Beta Reader Notification:**
```javascript
{
    type: "beta-reader",
    email: "reader@example.com",
    name: "Reader Name",
    bookTitle: "Murder, Malady and Monsters"
}
```

### Currently Using

| Feature | Uses Email Worker |
|---------|-------------------|
| Dots and Boxes | Game invites |
| Beta Reader Signups | Notification to author |
...
Added January 4, 2026 to support beta reader signups for The M Particle Series.

### Architecture

```
Website (murder-malady-monsters.html)
       |
       +---> Supabase (beta_readers table)
       |
       +---> Cloudflare Worker (notification email)
                    |
                    v
              stephenmbrandon@tecvoodoo.com
```
...
| Component | Purpose |
|-----------|---------|
| murder-malady-monsters.html | First 3 chapters preview + signup form |
| Supabase beta_readers table | Stores signups (email, name, book_title) |
| Cloudflare Worker | Sends notification to author |

### Flow

1. User reads first 3 chapters of M3
2. User clicks "Sign Up as Beta Reader" button
3. Modal appears with name/email form
4. On submit:
   - Record saved to Supabase `beta_readers` table
   - Notification email sent to author
   - Success message shown
5. Author receives email, manually sets up Google Drive folder for reader
...
- No user data storage (except via Supabase for games/beta readers)
- Static content only
- Supabase RLS for game data and beta reader signups
...
| Supabase | api.tecvoodoo.com | Game auth, multiplayer, beta reader signups |
| Cloudflare Worker | tecvoodoo-email.runeduvall.workers.dev | Game invites, beta reader notifications |
...
- Supabase: Required for multiplayer games and beta reader system
