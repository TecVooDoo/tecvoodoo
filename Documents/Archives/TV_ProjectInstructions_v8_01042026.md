...
└── site/                   # Website files (Cloudflare deploys this)
    ...
    ├── murder-malady-monsters.html  # M3 preview (added Jan 4, 2026)
    ├── css/
    │   └── style.css
    ├── images/
    ├── games/
...
### Add Beta Reader Signup for a Book

1. Create dedicated page (e.g., `site/book-title.html`) with:
   - First chapters preview
   - Beta reader signup modal
   - Supabase integration
2. Update `site/books.html` card with "Read Now" button linking to preview page
3. Book title is passed to Supabase `beta_readers` table and Cloudflare Worker
4. See `murder-malady-monsters.html` as reference implementation

```javascript
// Beta reader signup via Cloudflare Worker
const response = await fetch('https://tecvoodoo-email.runeduvall.workers.dev', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
...
| Beta reader duplicate error | Error code 23505 - same email already signed up for that book |
...
### Shared Worker for All Games and Books

All TecVooDoo games and books use the same Cloudflare Worker:
...
|------|-----------|---------|
| Game invite | Friend's email | Invite to play |
| Beta reader | stephenmbrandon@tecvoodoo.com | Notification of signup |
