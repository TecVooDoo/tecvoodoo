...
- Beta reader signup system for books
...
- Beta reader signups: Anyone can insert, only authenticated can read
...
The Supabase backend also supports the TecVooDoo website's beta reader signup system for books (The M Particle Series).

### How It Works

1. User visits tecvoodoo.com/books
2. Clicks "Read Now" on Murder, Malady and Monsters
3. Reads first 3 chapters preview
4. Signs up as beta reader (email + optional name)
5. Signup stored in `beta_readers` table
6. Cloudflare Worker sends notification email to author

### Integration

- Table: `beta_readers`
- RLS: Public insert, authenticated read
- Duplicate prevention: Unique index on (email, book_title)
...
- [x] Beta reader signup system (Jan 4, 2026)
