...
- `04_beta_readers` - Beta reader signup table
...
The `beta_readers` table stores signups from tecvoodoo.com/books.
...
- Only authenticated can SELECT (admin viewing)

### Integration

Signups come from the website via Supabase JS client. Email notifications sent via Cloudflare Worker.
...
| Beta reader duplicate | Error code 23505 - same email already signed up for that book |
...
|---------|-------------------|
| TecVooDoo Website | Auth test page, beta reader signups |
