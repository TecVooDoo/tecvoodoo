...
### Immediate
- [ ] **DAB email Worker hardening (security)** -- the `tecvoodoo-email` Worker is an open, unauthenticated relay (`Access-Control-Allow-Origin: *`, no origin check / auth / rate-limit): anyone can POST it to send the fixed invite/beta templates from `noreply@tecvoodoo.com` to any address with attacker-controlled name/link. Fix = Turnstile (see the `turnstile-spin` skill) + align the allowed origin with the itch port. Worker source is NOT in this repo -> needs wrangler + a deploy. (Deferred from 2026-07-10; the DAB stored-XSS half of batch #2 was fixed that session.)
- [ ] **Verify `auth-test.html` 404s in production** after the 2026-07-10 deploy (moved from `site/` to repo `dev/`, out of the Pages output dir).
