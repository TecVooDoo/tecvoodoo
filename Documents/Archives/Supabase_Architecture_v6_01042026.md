...
| v5 | Jan 4, 2026 | Custom domain (api.tecvoodoo.com), Pro tier, Facebook OAuth Live |
| v6 | Jan 4, 2026 | Added beta_readers table for book beta reader signups |
...
| Service | Purpose |
|---------|---------|
| PostgreSQL Database | Game state, player data, sessions, beta reader signups |
| Realtime | Live game updates via WebSocket subscriptions |
| Auth | Magic Link + Google OAuth + Facebook OAuth + Anonymous |
| Row Level Security | Data access control |
| Custom Domain | Professional OAuth consent screens |

### Project Details

| Property | Value |
|----------|-------|
| Project URL | https://api.tecvoodoo.com (custom domain) |
| Legacy URL | https://eenuxdjmsibcfhzgtslx.supabase.co |
...
| Cloudflare DNS | Custom domain CNAME and SSL verification |
| Cloudflare Worker | Email notifications for beta reader signups |
...
| matchmaking_queue | Matchmaking with phantom AI fallback | Enabled |
| games | Legacy Dots & Boxes table | Enabled |
| beta_readers | Book beta reader signups | Enabled |
...
| game_type | TEXT | - | Game to match for |
| player_name | TEXT | - | Display name |
| player_color | TEXT | - | Chosen color |
| queued_at | TIMESTAMPTZ | NOW() | When entered queue |
...
Stores beta reader signups from the Books page on tecvoodoo.com.

| Column | Type | Default | Purpose |
|--------|------|---------|---------|
| id | UUID | gen_random_uuid() | Primary key |
| email | TEXT | NOT NULL | Reader's email |
| name | TEXT | NULL | Reader's name (optional) |
| book_title | TEXT | NOT NULL | Which book they're signing up for |
| created_at | TIMESTAMPTZ | NOW() | Signup timestamp |

**Constraints:**
- UNIQUE INDEX on (email, book_title) - prevents duplicate signups for same book
...
|----------|--------|-------|
| Email (Magic Link) | Live | No password required |
| Google OAuth | Live | Via TecVooDoo GCP project |
| Facebook OAuth | Live | App Review completed Dec 28, 2025 |
| Anonymous | Live | Guest play without account |
...
| Authorized Origins | https://api.tecvoodoo.com |
| Callback URL | https://api.tecvoodoo.com/auth/v1/callback |
...
| Players can delete their game sessions | DELETE | created_by matches auth |
...
| Players can leave games | DELETE | player_id matches auth |
...
| Policy | Action | Rule |
|--------|--------|------|
| Anyone can sign up as beta reader | INSERT | true |
| Only authenticated users can view beta readers | SELECT | auth.role() = 'authenticated' |
...
| beta_readers_email_book_unique | beta_readers | email, book_title |
...
|----------|-------|---------------|
| Database Size | 8 GB | Minimal |
| Bandwidth | 50 GB/month | Minimal |
| Auth MAUs | 100,000 | Minimal |
| Realtime Connections | Included | Minimal |
| Edge Function Invocations | 2,000,000/month | 0 |
| Custom Domains | Included | api.tecvoodoo.com |
...
- `03_matchmaking_queue` - Creates matchmaking queue table
- `04_beta_readers` - Creates beta_readers table for book signups
