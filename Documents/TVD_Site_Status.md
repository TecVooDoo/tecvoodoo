...
- **Site→itch integration deferred to a dedicated session** (Rune's call): it involves *migrating/redoing some existing site games to run on itch* (real port work), after which the site gets itch links + a "Play in browser" path on the relevant game cards. Carryover noted above.
