...
## Deployment Structure Decision (Dec 26, 2025)
...
|--------|------|------|----------|
| Restructure local to match repo | Consistency | Loses organization | Rejected |
| Restructure repo to match local | Consistency | Breaking change, Cloudflare reconfigure | Rejected |
| Document the difference clearly | No changes needed | Requires awareness | CHOSEN |

### Resolution

- Keep local structure: `TecVooDooSite/site/` for working files
- Keep repo structure: files at root level
- Update documentation to clearly explain this
- Files are copied/staged from `site/` to repo root when committing

**Rationale:** The current setup works, changing either would risk breaking deployment. Documentation is the safest fix.
...
### From Deployment Structure Issue (Dec 26, 2025)

| Lesson | Application |
|--------|-------------|
| Local and repo structure can differ | Document it clearly |
| Don't assume folder structure | Check `git status` to see what's tracked |
| Working setup > perfect setup | If it works, document it rather than risk breaking it |
...
| Local vs repo structure mismatch | LOW | Documented, but could confuse new contributors |
